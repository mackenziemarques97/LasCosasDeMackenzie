...
Kp = abs(Kp(:));
x = displ_loading(0.75*length(displ_loading):end);
y = force_loading(0.75*length(force_loading):end);
p = polyfit(x, y, 1);
F1 = polyval(p,x);
Kr(j/5) = p(1);
Kr = abs(Kr(:));
