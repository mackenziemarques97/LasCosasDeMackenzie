...
pulsedurationVal = zeros(1,9);
Kp = zeros(1,9);%stiffness by pulse duration
Kr = zeros(1,9);%stiffness by regression
R = zeros(1,9);
R2 = zeros(1,9);
P = zeros(1,18); 
for j = 5:5:45
    filename=sprintf('OC%dcm1td1', j);
...
if autotrim == 1
    ...
    tl = 200; % Number of leading points to trim
    tt = 200; % Number of trailing points to trim
...
pulsedurationVal(j/5) = time(length(time))-time(1);
...
%Stiffness
Kp(j/5) = ((2.*pi./pulseduration).^2).*mass;
x = displ_loading(length(displ_loading)/2:end);
y = force_loading(length(force_loading)/2:end);
p = polyfit(x, y, 1);
F1 = polyval(p,x);
Kr(j/5) = p(1);
R(j/5) = corr(x,y);
        ...
        f = 1;
        Window = 5;
        while (f+Window < numel(x)) && keepgoing
            R2(j/5) = corr(x(f:f+Window), y(f:f+Window));
            if R2 > 0.97
                keepgoing = true;
                f = f+Window;
            elseif R2 < 0.97
...
figure(t)
plot(displ_loading,force_loading,displ_unloading,force_unloading,x,F1,X,F2,':','linewidth', 2)
axval = axis;
grid on
xlabel('Displacement [m]'), ylabel('Force [N]')
title(filename)
