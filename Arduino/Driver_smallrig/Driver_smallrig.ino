...
unsigned long microsteps = 8; /*divides the steps per revolution by this number, corresponds to microstepping settings on stepper driver, 16 corresponds to 3200 pulse/rev*/
unsigned long dimensions[2] = {1638 * microsteps, 2092 * microsteps}; /*preallocating dimensions to previously measured values*/
unsigned long location[2] = {0, 0}; /*presetting location*/
/*arbitrary upper limit for location input, with lower limit of 0*/
int virtDim = 100; /* max dimension that can be inputted*/

int vel = 150; /*default velocity for calibration and basic movement actions in terms of square pulse width (microseconds)*/
    ...
    switch ((int) *command) { /*switch case based on first command*/
      ...
      case 2: // move:x0:y0:hold duration
        ...
        */
        ...
        dispx = (long) (*(command + 1) / virtDim * dimensions[0]) - location[0]; /* Converting input virtual dimensions to microsteps*/
      ...
      case 3: // oscillate:x0:y0:x1:y1:speed:repetitions:resolution
        {
          ...
          */
          ...
          long dx = (long) ((*(command + 3) - * (command + 1)) / virtDim * dimensions[0]); /* Converting input virtual dimensions to microsteps*/
          ...
          */
          ...
          for (int j = spdi; j <= spdf; j += dspd) {
            ...
            for (int i = yi; i <= yf; i += dy) {
              ...
              recalibrate(yMin);
              delay(1000);
              int x = 1000; // Steps; why 1000? arbitrary?
              int y = i;
